...
optimize_all_years_nsga2 <- function(
  ...
  obs_x = NULL,        # only needed for current_local
  local_delta = 0.02   # +/- 15 percentage points around observed shares
  ...
  if (mode == "exploratory") {
    #  original wide bounds using historical min/max
    lower <- c(
      Corn     = crop_min["Corn"],
  ...
  } else if (mode == "current_local") {

    if (is.null(obs_x)) {
      stop("For mode = 'current_local', please provide obs_x (observed crop shares).")
    }

    # ensure correct order and drop any names weirdness
    x0 <- unname(obs_x[c("Corn","Soybeans","Sorghum")])

    # symmetric +/- local_delta around observed shares
    lower_raw <- pmax(0, x0 - local_delta)
    upper_raw <- pmin(1, x0 + local_delta)

    # still respect global historical min/max
    ...
    if (any(lower > upper)) {
      stop("Inconsistent bounds in current_local mode: some lower > upper. ",
           "Try reducing local_delta or checking crop_min/crop_max.")
    }

    message("Current-local bounds (Corn, Soybeans, Sorghum):")
    print(rbind(lower = lower, upper = upper))
  }
...
#run alg
  ...
  out$Nitrate_obj  <- res$value[, 1]   # mean nitrate (kg/yr) – minimize
  out$NR_obj      <- -res$value[, 2]  # mean net returns ($/yr) – maximize
  out$Irrig_m3_obj <- res$value[, 3]   # mean irrigation volume (m3/yr) – minimize
...
obs_point <- tibble(
  Nitrate_obj   = obs_vals[1],        # same name as in robust_out
  NR_obj        = -obs_vals[2],       # flip sign: profit (higher is better)
  Irrig_m3_obj  = obs_vals[3],
  type          = "Observed"
)
obs_point
obs_point <- as.data.frame(obs_point)
...
hist_obs_point <- df_int_basin %>%
  ...
  mutate(type = "Observed (hist)")

hist_obs_point <- as.data.frame(hist_obs_point)

#will have to rearrange, just seeing if it works for now
N_scale  <- hist_obs_point[1]  / obs_point[1]
NR_scale  <- hist_obs_point[2]  / obs_point[2]
Irr_scale <- hist_obs_point[3] / obs_point[3]


robust_out_curr_local <- robust_out %>%
  dplyr::mutate(type = "Optimized (local)") %>%
   mutate(
    Nitrate_obj   = as.numeric(Nitrate_obj)  * as.numeric(N_scale),
    NR_obj        = as.numeric(NR_obj)     * as.numeric(NR_scale),
    Irrig_m3_obj  = as.numeric(Irrig_m3_obj) * as.numeric(Irr_scale)
  ) %>%
  dplyr::bind_rows(hist_obs_point) %>%
  dplyr::bind_rows(obs_point %>% dplyr::mutate(
    Nitrate_obj   = as.numeric(Nitrate_obj)  * as.numeric(N_scale),
    NR_obj        = as.numeric(NR_obj)     * as.numeric(NR_scale),
    Irrig_m3_obj  = as.numeric(Irrig_m3_obj) * as.numeric(Irr_scale),
    type = "Observed (eval)"))
...
ggplot(robust_out_curr_local, aes(x = Nitrate_obj, y = NR_obj, color = Irrig_m3_obj)) +
  geom_point(aes(shape = type), size = 3.5, alpha = 0.8) +
...
tern_df <- robust_out_curr_local %>%
  ...
  mutate(
    ...
    econ_comp  = NR_scaled  / total,   # net returns
    wq_comp    = N_scaled   / total,   # water quality (low nitrate)
    water_comp = Irr_scaled / total    # water quantity (low irrigation)
  )
...
ggtern(
  ...
  geom_point(data = subset(tern_df, type == "Observed (hist)"),
             size = 3.5, shape = 20, fill = "white", stroke = 1) +
  labs(
    L = "Net returns\n(economics)",
    T = "Water quality\n(low N)",
    R = "Water quantity\n(low irr.)",
    title = "Observed system (df_opt) vs optimized frontier"
  ) +
  theme_bw() +
  theme_showarrows()

...
# Helper to get area in m2 from either m2 or ha column
get_area_m2 <- function(df, area_m2_col = "Crop_area_m2", area_ha_col = "Crop_area_ha") {
  df %>%
    mutate(
      area_m2 = dplyr::case_when(
...
# ---- 1) Land-use history from df_int_crop_areanorm --------------------------
# One unified object with area_m2 for all crops/years we care about
lu_hist <- df_int_crop_areanorm %>%
  filter(Crop %in% crops4) %>%
  get_area_m2() %>%
  drop_na(area_m2)

# (a) Historical min / max crop fractions over all years (for wide bounds)
hist_crop_frac <- lu_hist %>%
  group_by(Year, Crop) %>%
...
# (b) Current-period mean crop mix (for obs_x and "local" mode)
obs_mix <- lu_hist %>%
...
# Decision vector x = (Corn, Soy, Sorghum); Wheat implied
obs_x <- obs_mix %>%
...
obs_x
# implied_wheat <- 1 - sum(obs_x)

# ---- 2) Historical irrigation fractions per crop from df_opt ----------------
hist_mix <- tibble::tibble(
  Crop = crops4,
  # rough guess: corn most irrigated, then sorghum, then soy, then wheat
  irrig_frac = c(0.4, 0.55, 0.0, 0.0)
)
hist_mix
...
df_basin_input_forWQ <- df_basin_input_forWQ %>% left_join(df_nitrate, by = "Year") %>% subset(Year %in% years_curr)
...
# Training set for new model (ensure required cols exist)
req_wq_new <- c(
  "NitrateFlux_kg",
  ...
  "Management_FertilizerUse_kgm2"  # irrigation removed
)

wq_train <- wq_context %>% tidyr::drop_na(dplyr::all_of(req_wq_new))

# Fit the NEW model using your helper
...
eval_candidate <- function(
  ...
  x <- unname(x)  # <-- DROP any names from obs_x or other inputs
  ...
  perha <- perha %>%
  dplyr::mutate(fert_kgha = dplyr::coalesce(fert_kgha, 0))
  ...
  alloc <- allocate_irrigation(
    ...
    mode        = "preserve_hist_mix"   # i.e., same irrig fraction for all crops
  )
  ...
  perha_irrig <- perha %>%
    dplyr::mutate(
      ...
      # use climate/model-based depth per ha from df_opt
  ...
  pred_row <- tibble::tibble(
    ...
    # pass total fertilizer so add_wq_features() derives kg/m2 consistently
    Management_FertilizerUse_kg  = Fert_kg_total
  ) %>%
    dplyr::left_join(
      wq_context %>%
  ...
  # Only validate fields used by the fitted model
  req <- c("BasinArea_m2", "dev_frac", "cult_frac",
           "Climate_precip_m", "Climate_Tmean_C", "precip_percentileChange",
           "Management_FertilizerUse_kgm2")

  if (any(!is.finite(as.matrix(pred_row[req])))) return(c(PEN, PEN, PEN))

  nitrate_kgyr <- as.numeric(stats::predict(wq_model, newdata = pred_row))
  nitrate_kgyr <- max(0, nitrate_kgyr)
...
optimize_all_years_nsga2 <- function(
  ...
  seed = 123,
  mode = c("exploratory", "current_local"),
  obs_x = NULL,
  local_delta = 0.02
){
  stopifnot(length(years_vec) > 0)
  ...
  if (mode == "exploratory") {
    ...
    lower <- c(Corn = crop_min["Corn"],
               Soybeans = crop_min["Soybeans"],
               Sorghum = crop_min["Sorghum"])

    upper <- c(Corn = crop_max["Corn"],
               Soybeans = crop_max["Soybeans"],
               Sorghum = crop_max["Sorghum"])

  } else { # current_local

    if (is.null(obs_x)) stop("For mode = 'current_local', provide obs_x (observed crop shares).")

    x0 <- unname(obs_x[c("Corn","Soybeans","Sorghum")])

    lower_raw <- pmax(0, x0 - local_delta)
    upper_raw <- pmin(1, x0 + local_delta)

    lower_hist <- crop_min[c("Corn","Soybeans","Sorghum")]
    upper_hist <- crop_max[c("Corn","Soybeans","Sorghum")]

    lower <- pmax(lower_raw, lower_hist)
    upper <- pmin(upper_raw, upper_hist)

    names(lower) <- c("Corn","Soybeans","Sorghum")
    names(upper) <- c("Corn","Soybeans","Sorghum")

    if (any(lower > upper)) {
      stop("Inconsistent bounds in current_local mode: some lower > upper.")
    }
  }
  ...
  # IMPORTANT: assumes eval_candidate_all_years returns c(n_flux, neg_profit, irr)
  out$Nitrate_obj   <- res$value[, 1]
  out$NR_obj        <- -res$value[, 2]
  out$Irrig_m3_obj  <- res$value[, 3]
...
cols3     <- c("Nitrate_obj", "NR_obj", "Irrig_m3_obj")
...
robust_frontier <- robust_out[keep, ] %>%
  arrange(Nitrate_obj, desc(NR_obj), Irrig_m3_obj)
...
```
 ...
 - historical point v opt points
```{r}
# Observed point evaluated through the SAME eval pipeline as optimization
obs_vals <- eval_candidate_all_years(
  x                   = obs_x,
  years_vec           = years_curr,
  df_opt              = df_opt,
  wq_model            = wq_results$model,   # or wq_model if you store it separately
  wq_context          = wq_context,
  lu_baseline         = lu_baseline,
  universal_costs     = universal_costs,
  crop_params         = crop_params,
...
obs_point <- tibble::tibble(
  Nitrate_obj  = obs_vals[1],
  NR_obj       = -obs_vals[2],   # eval returns neg_profit; convert to +profit
  Irrig_m3_obj = obs_vals[3],
  type         = "Observed (eval)"
)



#hist_obs_point <- df_int_basin %>%
#  dplyr::filter(Year %in% years_curr) %>%
#  dplyr::summarise(
#    Nitrate_obj  = mean(NitrateFluxPredicted_kg, na.rm = TRUE),
#    NR_obj       = mean(NetReturn_total_usd,     na.rm = TRUE),
#    Irrig_m3_obj = mean(Irrigation_total_m3,     na.rm = TRUE),
#    .groups = "drop"
#  ) %>%
#  dplyr::mutate(type = "Observed (hist)")



robust_out_curr_local <- robust_out %>%
  dplyr::mutate(type = "Optimized (local)") %>%
#  dplyr::bind_rows(hist_obs_point) %>%
  dplyr::bind_rows(obs_point)
```
 ...
 - viz
```{r}
ggplot(robust_out_curr_local, aes(x = Nitrate_obj, y = NR_obj, color = Irrig_m3_obj)) +
  geom_point(
    data = subset(robust_out_curr_local, type == "Optimized (local)"),
    alpha = 0.6, size = 2.2
  ) +
  geom_point(
    data = subset(robust_out_curr_local, type != "Optimized (local)"),
  ...
  labs(
    ...
    title = "Observed vs optimized trade-offs"
  ) +
...
#install.packages('ggtern', repos = c('https://nicholasehamilton.r-universe.dev', 'https://cloud.r-project.org'))

library(ggtern)

tern_df <- robust_out_curr_local %>%
  dplyr::mutate(
    NR_scaled  = (NR_obj - min(NR_obj, na.rm = TRUE)) /
                 (max(NR_obj, na.rm = TRUE) - min(NR_obj, na.rm = TRUE) + 1e-9),

    # invert nitrate & irrigation so higher = better
    N_scaled   = (max(Nitrate_obj, na.rm = TRUE) - Nitrate_obj) /
                 (max(Nitrate_obj, na.rm = TRUE) - min(Nitrate_obj, na.rm = TRUE) + 1e-9),

    Irr_scaled = (max(Irrig_m3_obj, na.rm = TRUE) - Irrig_m3_obj) /
                 (max(Irrig_m3_obj, na.rm = TRUE) - min(Irrig_m3_obj, na.rm = TRUE) + 1e-9),

    total      = NR_scaled + N_scaled + Irr_scaled + 1e-12,
    econ_comp  = NR_scaled  / total,
    wq_comp    = N_scaled   / total,
    water_comp = Irr_scaled / total
  )

ggtern(
  data = tern_df,
  aes(x = econ_comp, y = wq_comp, z = water_comp)
) +
  geom_point(data = subset(tern_df, type == "Optimized (local)"),
             alpha = 0.6, size = 1.8) +
  geom_point(data = subset(tern_df, type == "Observed (eval)"),
             size = 3.5, shape = 21, fill = "white", stroke = 1) +
  geom_point(data = subset(tern_df, type == "Observed (hist)"),
             size = 3.5, shape = 20, stroke = 1) +
  labs(
    L = "Net returns\n(economics)",
    T = "Water quality\n(low N)",
    R = "Water quantity\n(low irr.)",
    title = "Observed system vs optimized frontier"
  ) +
  theme_bw() +
  theme_showarrows()
```

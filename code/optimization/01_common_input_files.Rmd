...
df_climate_county <- read_csv(here("data", "ClimateData_County.csv")) |>
...
```
 ...
 - need to check that the counties are clipped to watershed boundary!!! don't think they are...

## Land use
  - Similar to above
```{r}
#crop
df_landcover_county <- read_csv(here("data", "LandCoverData-CDL_County.csv")) |>
  mutate(
...
```{r}
#by crop
df_irrigation_in <- read_csv(here("data", "WaterUseData_County.csv")) |>
...
## Fertilizer 
  - right now just using constant, avg of rainfed and irr
...
- update: GDDs
```{r}
df_gdd <- read_csv(here("data", "gdd_all_gs.csv")) %>%
  filter(Year %in% yrs_common) |>
  mutate(
    FIPS = as.character(FIPS),
    spatial_scale = "county",
  ...
  dplyr::select(Year, FIPS, spatial_scale, Crop, GDD)
```
...
# Basin irrigation depth - crop specific -area-weighted mean depth
...
df_irr_basin <- cult_area_by_county_crop %>%
  ...
  group_by(Year) %>%
  summarise(
    # basin-average depth over corn+soy area
    Management_irrigation_m = weighted.mean(irrigation_WaterUse_m, w = area_m2, na.rm = TRUE),
    # also useful to keep:
...
```
 ...
 - save files 
```{r}
write_csv(df_combined,   here("data", "common_inputs_county.csv"))
write_csv(common_input_basin,  here("data","common_inputs_basin.csv"))
write_csv(df_county_areas,    here("data", "county_areas.csv"))

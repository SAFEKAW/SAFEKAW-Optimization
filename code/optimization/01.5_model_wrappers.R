...
fit_yield_models <- function(data, crop) {
  ...
  # Make grouping vars factors (safer for lmer + predict)
  dfc$FIPS <- as.factor(dfc$FIPS)
  dfc$Year <- as.factor(dfc$Year)
  
  # Fixed-effects part: same as before
  if (crop == "Sorghum") {
    fixed_kcal <- yield_kcalHa_detrended ~ poly(totalWater_m, 2) + GDD
    fixed_kg   <- yield_kgHa_detrended   ~ poly(totalWater_m, 2) + GDD
  } else {
    fixed_kcal <- yield_kcalHa_detrended ~ poly(totalWater_m, 2) + GDD
    fixed_kg   <- yield_kgHa_detrended   ~ poly(totalWater_m, 2) + GDD
  }
  ...
  formula_kcal <- update(fixed_kcal, . ~ . + (1 | FIPS) +(1 | Year) ) #(1 | Year)
  formula_kg   <- update(fixed_kg,   . ~ . + (1 | FIPS) + (1 | Year)) #+ (1 | Year)
  
  fit_kcal <- lme4::lmer(formula_kcal, data = dfc, REML = TRUE)
  fit_kg   <- lme4::lmer(formula_kg,   data = dfc, REML = TRUE)
  
  
  # ---- Predictions on fitted data ----
  # Conditional (includes random effects) == fitted()
  dfc$kcal_fit_cond <- as.numeric(predict(fit_kcal))
  dfc$kg_fit_cond   <- as.numeric(predict(fit_kg))
  
  # Fixed-only (population-level; excludes random effects)
  dfc$kcal_fit_fixed <- as.numeric(predict(fit_kcal, re.form = NA))
  dfc$kg_fit_fixed   <- as.numeric(predict(fit_kg,   re.form = NA))
  
  # Keep your original fitted columns too (optional)
  dfc$yield_kcalHa_detrended_fit <- dfc$kcal_fit_cond
  dfc$yield_kgHa_detrended_fit   <- dfc$kg_fit_cond
  
  # ---- Helper metrics ----
  rmse <- function(obs, pred) sqrt(mean((pred - obs)^2, na.rm = TRUE))
  bias <- function(obs, pred) mean(pred - obs, na.rm = TRUE)  # positive = overprediction
  
  # Complete-case masks (so metrics are apples-to-apples)
  ok_kcal_cond  <- complete.cases(dfc$yield_kcalHa_detrended, dfc$kcal_fit_cond)
  ok_kcal_fixed <- complete.cases(dfc$yield_kcalHa_detrended, dfc$kcal_fit_fixed)
  ok_kg_cond    <- complete.cases(dfc$yield_kgHa_detrended,   dfc$kg_fit_cond)
  ok_kg_fixed   <- complete.cases(dfc$yield_kgHa_detrended,   dfc$kg_fit_fixed)
  
  # R² (obs vs fitted) — conditional
  r2_kcal <- with(dfc[ok_kcal_cond, ],
                  cor(yield_kcalHa_detrended, kcal_fit_cond)^2)
  r2_kg   <- with(dfc[ok_kg_cond, ],
                  cor(yield_kgHa_detrended, kg_fit_cond)^2)
  
  # RMSE + bias — conditional and fixed-only
  metrics <- list(
    kcal = c(
      R2_cond   = r2_kcal,
      RMSE_cond = rmse(dfc$yield_kcalHa_detrended[ok_kcal_cond], dfc$kcal_fit_cond[ok_kcal_cond]),
      Bias_cond = bias(dfc$yield_kcalHa_detrended[ok_kcal_cond], dfc$kcal_fit_cond[ok_kcal_cond]),
      RMSE_fixed = rmse(dfc$yield_kcalHa_detrended[ok_kcal_fixed], dfc$kcal_fit_fixed[ok_kcal_fixed]),
      Bias_fixed = bias(dfc$yield_kcalHa_detrended[ok_kcal_fixed], dfc$kcal_fit_fixed[ok_kcal_fixed])
    ),
    kg = c(
      R2_cond   = r2_kg,
      RMSE_cond = rmse(dfc$yield_kgHa_detrended[ok_kg_cond], dfc$kg_fit_cond[ok_kg_cond]),
      Bias_cond = bias(dfc$yield_kgHa_detrended[ok_kg_cond], dfc$kg_fit_cond[ok_kg_cond]),
      RMSE_fixed = rmse(dfc$yield_kgHa_detrended[ok_kg_fixed], dfc$kg_fit_fixed[ok_kg_fixed]),
      Bias_fixed = bias(dfc$yield_kgHa_detrended[ok_kg_fixed], dfc$kg_fit_fixed[ok_kg_fixed])
    )
  )
...
run_crop_models <- function(df) {
  ...
  # Extract fit stats into a table (one row per crop)
  df_fit <- data.frame(
    Crop = sapply(results, `[[`, "crop"),
    
    fit_R2_kcalHa = sapply(results, function(x) unname(x$metrics$kcal["R2_cond"])),
    rmse_kcal_cond = sapply(results, function(x) unname(x$metrics$kcal["RMSE_cond"])),
    bias_kcal_cond = sapply(results, function(x) unname(x$metrics$kcal["Bias_cond"])),
    rmse_kcal_fixed = sapply(results, function(x) unname(x$metrics$kcal["RMSE_fixed"])),
    bias_kcal_fixed = sapply(results, function(x) unname(x$metrics$kcal["Bias_fixed"])),
    
    fit_R2_kgHa   = sapply(results, function(x) unname(x$metrics$kg["R2_cond"])),
    rmse_kg_cond  = sapply(results, function(x) unname(x$metrics$kg["RMSE_cond"])),
    bias_kg_cond  = sapply(results, function(x) unname(x$metrics$kg["Bias_cond"])),
    rmse_kg_fixed = sapply(results, function(x) unname(x$metrics$kg["RMSE_fixed"])),
    bias_kg_fixed = sapply(results, function(x) unname(x$metrics$kg["Bias_fixed"]))
  )
...
add_wq_features <- function(df) {
  ...
  # Ensure required columns exist (as NA) so mutate/coalesce won't error
  ...
  # Optional Tmean fallback ONLY if Tmin/Tmax exist
  if (all(c("Climate_Tmin_C", "Climate_Tmax_C") %in% names(df))) {
    df <- df %>%
      dplyr::mutate(
        Climate_Tmean_C = dplyr::coalesce(
          Climate_Tmean_C,
          (Climate_Tmin_C + Climate_Tmax_C) / 2
        )
      )
  }
  
  # Fractions + fertilizer density
  df <- df %>%
    dplyr::mutate(
      grass_frac  = dplyr::if_else(BasinArea_m2 > 0,
                                   LandCover_m2_grassPastureHay / BasinArea_m2,
                                 NA_real_),
      cult_frac = dplyr::if_else(BasinArea_m2 > 0,
                                 `LandCover_m2_all cult` / BasinArea_m2,
                                 NA_real_),
      
      Management_FertilizerUse_kgm2 = dplyr::coalesce(
        Management_FertilizerUse_kgm2,
        dplyr::if_else(
          !is.na(Management_FertilizerUse_kg) & BasinArea_m2 > 0,
...
run_waterquality_model <- function(df) {
  ...
  list(
    data = df,
    model = mlr_nitrate,
    performance = list(MAE = mae_val, NMAE = nmae_val, R2 = R2_val)
  )
}
...
# irr_aug: county rows with irrigation_WaterUse_m and area_m2
  ...
  compute_net_returns <- function(df_rows, crop_params, universal_costs) {
    ...
    # If m3/ha not present but depth is, convert depth (m) -> m3/ha
    if (!"Irrigation_m3Ha_pred" %in% names(df_rows) && "irr_depth_m_pred" %in% names(df_rows)) {
      df_rows <- df_rows %>%
        dplyr::mutate(Irrigation_m3Ha_pred = dplyr::coalesce(irr_depth_m_pred, 0) * 10000)
    }

...
df_climate_county <- read_csv(file.path("data", "ClimateData_County.csv")) |>
  ...
  summarise(
    ...
    precip_m_avg  = mean(precip_mm_gridmet)/1000,    # mean depth (m) — keep if you use it
...
#save files 
```{r}
write_csv(df_combined,          "common_inputs_county.csv")
write_csv(common_input_basin,   "common_inputs_basin.csv")
write_csv(df_county_areas,      "county_areas.csv")

...
After a successful run, see [Where to find results and figures](#where-to-find-results-and-figures) for the numerical outputs, generated plots, and scripts used to recreate them.
...
### Where to find results and figures

The scenario manifest is:

- `hpc_opt/outputs/deterministic_factorial_grid_32_scenarios.csv`

It identifies all 32 factorial combinations and the corresponding result directory for each combination. The individual scenario directories are under:

- `hpc_opt/outputs/factorial_runs/`

Each scenario directory contains:

- `deterministic_results.csv`: annual results for the complete future trajectory;
- `summary_by_period.csv`: early-, mid-, and late-century summaries;
- `summary_overall.csv`: a whole-trajectory summary;
- `irrigation_by_crop.csv`: crop-level irrigation results;
- `scenario_path.csv`: the land-use and management trajectory evaluated.

The main deterministic figures are written to:

- `hpc_opt/outputs/factorial_runs/figures/`

These figures are generated by:

- `hpc_opt/scripts/diagnostics/05_check_deterministic_factorial.R`

The packaged deterministic runner executes this script automatically. To regenerate the figures without rerunning the 32 scenarios, source it from an R session at the repository root:

```r
source(here::here(
  "hpc_opt", "scripts", "diagnostics",
  "05_check_deterministic_factorial.R"
))
```

The paired deterministic counterfactual checks are generated by:

- `hpc_opt/scripts/diagnostics/05_check_deterministic_counterfactuals.R`

For a focused table comparing historical conditions with fixed- and BAU-land-use future projections under current management, run:

- `hpc_opt/scripts/diagnostics/05_summarize_forward_projection.R`

That script writes its tables under:

- `hpc_opt/outputs/deterministic_summary/`

The other `05_viz_*` scripts are specialized analyses associated primarily with optimization results. They are not required to inspect or plot the standard deterministic factorial run.
